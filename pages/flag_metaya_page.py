...
from Flag.analyzer_core import analyze

st.title(" Flag  — Tax Risk Analyzer")
...
use_kg = opt1.toggle("Knowledge Graph", help="Semantic KG retrieval (Neo4j + bge-m3)")
think  = opt2.toggle(
    "Thinking Mode",
...
with col2:
    if st.button("Load example Rivera2024", use_container_width=True):
        st.session_state["_fm_json_text"] = json.dumps(EXAMPLE, indent=2)
with col3:
    if st.button("Load example IRA", use_container_width=True):
        st.session_state["_fm_json_text"] = json.dumps(EXAMPLE_IRA, indent=2)
...
if run:
    ...
    st.session_state["_fm_last_result"] = result

# ── Results ────────────────────────────────────────────────────────────────
result = st.session_state.get("_fm_last_result")
if not result:
    st.stop()
...
for f in flags:
    ...
    with st.expander(f"{icon} [{f.get('flag_id','')}] {f.get('flag_title','')}  —  {risk.upper()}", expanded=(risk == "high")):

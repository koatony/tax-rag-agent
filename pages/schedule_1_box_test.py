...
# --- 頁面配置 ---
st.set_page_config(
    page_title="Schedule 1 表單盒動態提取與計算測試",
    layout="wide"
)
...
with st.sidebar:
    ...
    st.divider()
    st.info("💡 說明：\n1. 本系統會動態載入外部的 `schedule_1_schema.json` 規則設定檔。\n2. LLM 只負責從原始資料中定性提取 Input 欄位。\n3. Python 圖論計算引擎會自動解析依賴，完成公式求值。")

# --- 主畫面標題 ---
st.title("📋 IRS Schedule 1 表單盒動態提取與計算測試")
st.write("此頁面用於測試將整個 Schedule 1 作為單一盒子 (Box) 進行填寫。LLM 會閱讀所有原始憑證資料，依據外部 JSON 規格提取 Input 欄位，再由 Python 自動執行排序與公式計算。")

# 初始化 Session 狀態
if "rivera_input_text_1" not in st.session_state:
    st.session_state.rivera_input_text_1 = ""

if "test_execution_result_1" not in st.session_state:
    st.session_state.test_execution_result_1 = None
...
with col_btn1:
    if st.button("📥 載入 Sample 1~10（JSON）", use_container_width=True, help="載入 data/src_json_Marcus_and_Elena/ 下的 JSON 實驗資料"):
        st.session_state.rivera_input_text_1 = schedule_a_ui_helper.load_all_rivera_samples()
        st.rerun()
with col_btn2:
    if st.button("📄 載入 v2 0622（Word 原始憑證）", use_container_width=True, help="載入 Sample Data Pack v2 0622 的 .docx 文件"):
        st.session_state.rivera_input_text_1 = load_sample_data_pack_v2()
        st.rerun()

# 顯示輸入框
prompt_input = st.text_area(
    "請輸入原始報稅資料 (JSON 或 Markdown 文字列表)：",
    value=st.session_state.rivera_input_text_1,
    height=280,
    placeholder="在此輸入或點擊上方按鈕載入模擬的 Word 憑證資料..."
)
st.session_state.rivera_input_text_1 = prompt_input
    ...
    if not prompt_input.strip():
        ...
        with st.spinner("正在呼叫 LLM 進行欄位提取並執行拓撲公式計算..."):
            ...
            try:
                ...
                # 4. 儲存結果
                st.session_state.test_execution_result_1 = {
...
# --- 顯示結果 ---
if st.session_state.test_execution_result_1:
    res = st.session_state.test_execution_result_1
    ...
    with tab_report:
        st.write("以下為 Schedule 1 欄位名稱與對應欄位值：")

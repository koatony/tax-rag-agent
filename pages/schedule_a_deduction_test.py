...
# ─── 頁面配置 ─────────────────────────────────────────────────────────────
st.set_page_config(
    page_title="Schedule A — Itemized Deductions Adapter & Mapper 測試",
    layout="wide",
)
...
with st.sidebar:
    ...
    st.markdown(
        ...
        **📐 架構說明**
        ...
        ```
        扣除文件 (OCR / 1098 / 收據)
           ↓  ItemizedDeductionAdapter (LLM)
        Deduction Facts
           ↓  ScheduleADeductionMapper (LLM)
        Mapped Items & Unmapped Items
        ```

        - **ItemizedDeductionAdapter**：提取原子事實
        - **Mapper**：映射至 Schedule A 行號或保留在 unmapped_items（政治捐款排除）
        """
    )

# ─── 主畫面標題 ──────────────────────────────────────────────────────────
st.title("📋 Schedule A — Itemized Deductions Adapter & Mapper 測試")
st.markdown(
    "此頁面測試 **User Story 5.3 — Map Itemized Deductions to Schedule A**：\n"
    "從扣除文件中提取事實並映射至 Schedule A 行號（Line 11, Line 8a, Line 5b 等），同時排除政治捐款。"
)

# ─── Session State 初始化 ─────────────────────────────────────────────────
if "schedule_a_input_json" not in st.session_state:
    st.session_state.schedule_a_input_json = ""

if "schedule_a_result" not in st.session_state:
    st.session_state.schedule_a_result = None

# ─── 載入測試資料按鈕 ─────────────────────────────────────────────────────
col_btn, _ = st.columns([2, 5])
with col_btn:
    if st.button("📥 載入 Rivera 夫婦扣除項目測試資料", use_container_width=True, key="load_rivera_btn"):
        val_str = json.dumps(
            DEFAULT_DEDUCTION_INPUT, indent=2, ensure_ascii=False
        )
        st.session_state.schedule_a_input_json = val_str
        st.session_state.schedule_a_text_area = val_str
        st.rerun()

# ─── 輸入區 ───────────────────────────────────────────────────────────────
prompt_input = st.text_area(
    "請輸入扣除文件清單 JSON：",
    value=st.session_state.schedule_a_input_json,
    height=260,
    placeholder="點擊上方按鈕載入 Rivera 夫婦測試資料，或自行貼入扣除文件 JSON...",
    key="schedule_a_text_area",
)
st.session_state.schedule_a_input_json = prompt_input
    ...
    if not prompt_input.strip():
        ...
        if not api_key:
            ...
            try:
                ...
                total_docs = len(docs)
                if total_docs == 0:
                    st.error("未找到任何扣除文件 (documents)！")
                else:
                    # ── Step 1: 並發呼叫 ItemizedDeductionAdapter ─────────
                    progress_container = st.container()
                    with progress_container:
                        st.markdown(
                            '<span class="step-badge">Step 1</span> ItemizedDeductionAdapter — 並發提取原子事實',
                            unsafe_allow_html=True,
                        )
                    ...
                    # ── Step 2: 呼叫 ScheduleADeductionMapper ──────────────
                    status_text.text("Adapter 提取完成，正在呼叫 ScheduleADeductionMapper...")
                    st.markdown(
                        '<span class="step-badge">Step 2</span> ScheduleADeductionMapper — 映射至 Schedule A 行號',
                        unsafe_allow_html=True,
                    )
                    ...
                    # 儲存結果
                    st.session_state.schedule_a_result = {
...
# ─── 顯示結果 ─────────────────────────────────────────────────────────────
if st.session_state.schedule_a_result:
    res = st.session_state.schedule_a_result
    ...
    # ─── Form 1040 Schedule A 最終結果卡片 ──────────────────────────────
    st.markdown("### 🧾 Schedule A — 映射與排除明細")

    source_items = mapper_result.get("items") or []
    unmapped_items = mapper_result.get("unmapped_items") or []
    needs_review = mapper_result.get("needs_review", False)
    ...
    st.markdown(
        f"""
        <div class="schedule-a-card">
            <div style="margin-bottom:10px; color:#94a3b8; font-size:0.9rem; font-weight:600;">
                IRS Schedule A &nbsp;›&nbsp; <span style="color:#a5b4fc;">Itemized Deductions</span>
                &nbsp;&nbsp;{status_badge}
            </div>
        </div>
        """,
    ...
    if needs_review:
        st.markdown(
            ...
            <div class="review-warning-box">
                ...
                請至 Debug 區或下方明細表格中查看各項目的 needs_review 與 review_reason。
            </div>
            """,
            unsafe_allow_html=True,
        )
    ...
    # ─── 映射明細表格 ──────────────────────────────────────────────────
    st.markdown("#### 📋 Schedule A 映射明細 (items)")

    if not source_items:
        st.warning("沒有成功映射的 Schedule A 項目。")
    else:
        html_rows = []
    ...
    # ─── 排除/未映射項目 ────────────────────────────────────────────────
    st.markdown("#### 🚫 排除與未映射項目 (unmapped_items)")
    if not unmapped_items:
        st.info("沒有被排除或未映射的項目。")
    else:
        html_rows_unmapped = []
    ...
    # AC1: Church donation 被映射至 Schedule A Line 11 ($5,400)
    church_item = next(
        (i for i in source_items if i.get("target_line") == "line_11"),
        None,
    )
    ac1_pass = church_item is not None and abs(church_item.get("value", 0.0) - 5400.0) < 0.01
    ac_rows.append(
        (
            "Church donation mapped to Schedule A Line 11 ($5,400)",
            f"✅ ${church_item['value']:,.2f}" if ac1_pass else f"❌ 實際值 {church_item['value'] if church_item else '未找到'}",
            ac1_pass,
        )
    )

    # AC2: Home mortgage interest 被映射至 Schedule A Line 8a ($9,800)
    mortgage_item = next(
        (i for i in source_items if i.get("target_line") == "line_8a"),
        None,
    )
    ac2_pass = mortgage_item is not None and abs(mortgage_item.get("value", 0.0) - 9800.0) < 0.01
    ac_rows.append(
        (
            "Home mortgage interest mapped to Schedule A Line 8a ($9,800)",
            f"✅ ${mortgage_item['value']:,.2f}" if ac2_pass else f"❌ 實際值 {mortgage_item['value'] if mortgage_item else '未找到'}",
            ac2_pass,
        )
    )

    # AC3: Personal real estate taxes 被映射至 Schedule A Line 5b ($2,600)
    tax_item = next(
        (i for i in source_items if i.get("target_line") == "line_5b"),
        None,
    )
    ac3_pass = tax_item is not None and abs(tax_item.get("value", 0.0) - 2600.0) < 0.01
    ac_rows.append(
        (
            "Personal real estate taxes mapped to Schedule A Line 5b ($2,600)",
            f"✅ ${tax_item['value']:,.2f}" if ac3_pass else f"❌ 實際值 {tax_item['value'] if tax_item else '未找到'}",
            ac3_pass,
        )
    )

    # AC4: Political contribution 被排除，不得進入 Schedule A
    political_mapped = next(
        (i for i in source_items if i.get("source_fact_type") == "political_contribution"),
        None,
    )
    ac4_pass = political_mapped is None
    ac_rows.append(
        (
            "Political contribution excluded from Schedule A lines",
            "✅ 已成功排除" if ac4_pass else "❌ 錯誤映射到 Schedule A",
            ac4_pass,
        )
    )

    # AC5: 被排除的 political contribution 保留在 unmapped_items，便於來源追蹤
    political_unmapped = next(
        (i for i in unmapped_items if i.get("source_fact_type") == "political_contribution"),
        None,
    )
    ac5_pass = (
        political_unmapped is not None
        and abs(political_unmapped.get("value", 0.0) - 250.0) < 0.01
        and political_unmapped.get("review_reason") == "political_contribution_non_deductible"
    )
    ac_rows.append(
        (
            "Political contribution retained in unmapped_items ($250, excluded)",
            f"✅ 已保留於 unmapped_items 且 reason 正確" if ac5_pass else "❌ 未找到或狀態不符",
            ac5_pass,
        )
    )
    ...
    if is_debug:
        ...
        tab_adapter, tab_mapper = st.tabs(
            [
                "1. ItemizedDeductionAdapter 偵錯",
                "2. ScheduleADeductionMapper 偵錯",
            ]
        )
        ...
        # Tab 2 — Mapper
        with tab_mapper:
            st.markdown("#### 📋 ScheduleADeductionMapper 映射結果 JSON")

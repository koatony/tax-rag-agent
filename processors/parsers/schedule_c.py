...
class ScheduleCLLMParser(BaseLLMParser):
    ...
    def get_custom_rules(self) -> List[str]:
        return [
            ...
            "對於旗標 'cogs_module_completed'，只有在輸入文件中明確看見已經計算完成的銷貨成本 (COGS) 具體數字（如損益表中的 Cost of Goods Sold 總額）且填入 'line_4_cogs_from_module' 時，此旗標才可設定為 true。若文件中沒有直接提供計算好的 COGS 最終數字（例如只有零散進貨發票、存貨清單，或該欄位為空），此旗標必須設為 false。"
        ]

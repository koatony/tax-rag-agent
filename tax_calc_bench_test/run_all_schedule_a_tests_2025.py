...
cases_target_dir = os.path.join(target_dir, "cases_2025")
...
def main():
    ...
    md_lines = []
    md_lines.append("# 📋 TaxCalcBench (TY25) Schedule A 完整自動化對齊測試報告")
    md_lines.append(f"\n- **測試執行時間**: {time.strftime('%Y-%m-%d')}")
    ...
    for r in results_summary:
        ...
        md_lines.append(f"| [{case_name}](cases_2025/{case_name}/) | {status} | {ded_str} | {item_str} | {r['latency']:.2f} |")
        
    md_lines.append("\n## 📂 檔案目錄說明")
    md_lines.append("- 各案例的來源與輸出保存在 `cases_2025/` 資料夾中。")
    ...
    # 寫入 log_2025.md
    report_path = os.path.join(target_dir, "log_2025.md")

...
def main():
    ...
    # 目標紀錄資料夾
    target_dir = os.path.join(workspace_dir, "tax_calc_bench_test", "schedule_a")
    cases_target_dir = os.path.join(target_dir, "cases")
    os.makedirs(cases_target_dir, exist_ok=True)
    
    print(f"=== 開始執行 Schedule A 批次對齊測試 (總共 {len(SCH_A_CASES)} 個案例) ===")
    ...
    for case in SCH_A_CASES:
        ...
        try:
            ...
            # 呼叫 LLM 進行提取
            extracted_inputs, _, _ = extract_schedule_a_inputs_with_logs(text_context, model_name="gemini-2.5-flash")
    ...
    md_lines = []
    md_lines.append("# 📋 TaxCalcBench (TY24) Schedule A 完整自動化對齊測試報告")
    ...
    for r in results_summary:
        ...
        md_lines.append(f"| [{case_name}](cases/{case_name}/) | {status} | {ded_str} | {item_str} | {r['latency']:.2f} |")
